...
//这个sort类是为了实现倒叙输出，不然就会导致value小的在后面
static 	class KpiSort extends Comparator{
	...
	public static  Job getJob2() throws Exception {
		...
		String inputPath="/user/data/kpi/ips/out/part-r-00000";
		String outputPath="/user/data/kpi/ips/out/sort";
		FileSystem fs=FileSystem.get(conf);
		if(fs.exists(new Path(outputPath))){
			fs.delete(new Path(outputPath), true);
		}	
		//构建新的作业
		Job job = Job.getInstance(conf, "IPS second Reduce");
