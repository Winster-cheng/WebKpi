...
public class SecondReduce extends Configured {
	...
	static class TimeSecondReducer extends Reducer<IntWritable, Text, Text, IntWritable>{
		@Override
	...
	public static  Job getJob2() throws Exception {
		...
		String inputPath="/user/data/kpi/time/out/part-r-00000";
		String outputPath="/user/data/kpi/time/out/sort";
		FileSystem fs=FileSystem.get(conf);
		if(fs.exists(new Path(outputPath))){
			fs.delete(new Path(outputPath), true);
		}	
		//构建新的作业
		Job job = Job.getInstance(conf, "time second");
		job.setJarByClass(SecondReduce.class);
		
 		//给job设置mapper类及map方法输出的键值类型
		job.setMapperClass(TimeSecondMapper.class);
		job.setMapOutputKeyClass(IntWritable.class);
		job.setMapOutputValueClass(Text.class);
		
		//给job设置reducer类及reduce方法输出的键值类型
		job.setReducerClass(TimeSecondReducer.class);
